...
{	
	...
	//判断智能体到达目标所需时间,Arrive用到
	public enum Decelerate{slow=3,normal=2,fast=1};
	...
	public Vector3 GetSeekVector(Vector3 target)
	{
		...
		Vector3 desiredVel = difference * _maxSpeed;
			/*
	...
	//到达
	public Vector3 GetArriveVector(Vector3 target,Decelerate decel)
	{
		...
		if (dist > 8) {
			const float decelWeaker=0.3f;
			float  speed=dist/((float)decel*decelWeaker);
			speed=Mathf.Min(speed,_maxSpeed);
			Vector3 desiredVel=difference*speed/dist;
